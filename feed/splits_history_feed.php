...
$count_sql = "SELECT count(*) as count FROM `stock_feed` WHERE `Outcome` LIKE 'Success' && `Date` != ''  && `uid` > '20904';";
...
for ($loop=1; $loop <=$num_of_requests; $loop++)
    {
    $string = "";
    $symbol_sql = "SELECT * FROM `stock_feed` WHERE `Outcome` LIKE 'Success' && `Date` != ''  && `uid` > '20904' LIMIT $step, $results_per_request;";
    ...
    $string = rtrim($string, ",");
    include "splits_json_engine.php";
