...
$today = date("Y/m/d")."%";
$sql = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'TKOVR' AND `changed` LIKE '$today' AND `exchgcntry` LIKE 'US' AND `Field20` NOT LIKE '';";
        ...
        switch ($paytype) {
            ...
            case "C":
                ...
                print "<br>------------------------------------------";
                    ...
                    //create line of csv
                    print "_____MATCHES_____";
                    $csv .= $_695localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$date4.'|ACQUIS|'.$actflag.'|'.$_695issuername.'|N.A.|N.A.|N.A.| | |'.$created.'|'.$date4.'|'.$changed.'|N.A.| |N.A.|N.A.|13|CP_TYP|2|CP_CASH|'.$rate2.'|CP_CASH_FLAG|1|CP_SH|0|CP_SH_FLAG|N.A.|CP_FLAG|1|CP_TKR| |CP_NAME|'.$field3.'|CP_STAT|3|CP_CRNCY|'.$currency.'|CP_DEBT|0|CP_DEBT_FLAG| |CP_NOTES|N.A.|' . "\r\n";
