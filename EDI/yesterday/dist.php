...
for ($counter=1; $counter <= mysqli_num_rows($result); $counter++)
        ...
        $csv .= $_695localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$date3.'|DVD_STOCK|'.$actflag.'|'.$_695issuername.'|CUSIP|'.$uscode.'|USD|Equity| |'.$created.'|'.$date1.'|'.$changed.'|'.$bbgcompositeglobalid.'| |'.$_695localcode.'|US|11|CP_RECORD_DT|'.$date2.'|CP_PAY_DT|'.$date3.'|CP_GROSS_AMT|'.$rate1.'|CP_NET_AMT|'.$rate2.'|CP_FREQ| |CP_ADJ|'.$CP_ADJ.'|CP_ADJ_DATE|'.$date1.'|CP_TAX_AMT|N.A.|CP_DVD_CRNCY|'.$currency.'|CP_DVD_TYP|0|CP_NOTES|N.A.|' . "\r\n";
        }
//print "hi";
