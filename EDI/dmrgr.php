...
$custom = "2016/08/19%";
$today = date("Y/m/d")."%";
$sql = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'DMRGR' AND `changed` LIKE '$today' AND `exchgcntry` LIKE 'US' AND `OutturnIsin` LIKE 'US%';";
...
for ($counter=1; $counter <= mysqli_num_rows($result); $counter++)
        ...
        $csv .= $localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$created.'|SPIN|'.$actflag.'|'.$issuername.'|CUSIP|'.$uscode.'|USD|Equity| |'.$created.'|'.$date1.'|'.$changed.'|'.$bbgcompositeglobalid.'| |'.$localcode.'|US|7|CP_PAY_DT|'.$date3.'|CP_RECORD_DT|'.$date2.'|CP_SPINOFF_NAME|'.$cp_spinoff_name.'|CP_TKR|'.$field20.'|CP_TERMS|'.$rationew.' per '.$ratioold.'|CP_RATIO|'.$CP_RATIO.'|CP_NOTES|N.A.|' . "\r\n";
        }
//print "hi";
