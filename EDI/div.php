...
$today = date("Y/m/d")."%";
$sql = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'DIV' AND `changed` LIKE '$today' AND `exchgcntry` LIKE 'US'";
print $sql;
        ...
        switch ($paytype) {
            case "S":
                ...
                print "<br>------------------------------------------";
                    ...
                    $csv .= $_695localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$created.'|DVD_STOCK|'.$actflag.'|'.$_695issuername.'|CUSIP|'.$uscode.'|USD|Equity| |'.$created.'|'.$date1.'|'.$changed.'|'.$bbgcompositeglobalid.'||'.$_695localcode.'|US|11|CP_AMT|'.$CP_AMT.'|CP_TKR|'.$_695localcode.' US|CP_RECORD_DT|'.$date2.'|CP_PAY_DT|'.$date3.'|CP_FREQ| |CP_ADJ|'.$rationew.'|CP_ADJ_DATE|'.$date1.'|CP_TAX_AMT|N.A.|CP_DVD_CRNCY|'.$currency.'|CP_DVD_TYP|0|CP_NOTES|N.A.| ' . "\r\n";
            ...
            case "C":
                ...
                print "<br>------------------------------------------";
                    ...
                    //create line of csv
                    print "_____MATCHES_____";
                    $csv .= $_695localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$created.'|DVD_CASH|'.$actflag.'|'.$_695issuername.'|CUSIP|'.$uscode.'|USD|Equity| |'.$created.'|'.$date1.'|'.$changed.'|'.$bbgcompositeglobalid.'||'.$_695localcode.'|US|9|CP_RECORD_DT|'.$date2.'|CP_PAY_DT|'.$date3.'|CP_GROSS_AMT|'.$rate1.'|CP_NET_AMT|'.$rate2.'|CP_FREQ| |CP_TAX_AMT|N.A.|CP_DVD_CRNCY|'.$currency.'|CP_DVD_TYP|0|CP_NOTES|N.A.|' . "\r\n";
