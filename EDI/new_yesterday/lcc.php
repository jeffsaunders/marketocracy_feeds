...
//$sql = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'LCC' AND `changed` LIKE '2016/10/11%' AND `exchgcntry` LIKE 'US';";
$yesterday = date("Y/m/d", strtotime( '-1 days' ) )."%";
$custom = "2016/08/18%";
$today = date("Y/m/d")."%";
$sql = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'LCC' AND `changed` LIKE '$yesterday' AND `exchgcntry` LIKE 'US';";
...
for ($counter=1; $counter <= mysqli_num_rows($result); $counter++)
        ...
        $csv .= $field2.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$created.'|CHG_TKR|'.$actflag.'|'.$_695issuername.'|CUSIP|'.$uscode.'|USD|Equity| |'.$created.'|'.$date1.'|'.$changed.'|N.A.|'.$bbgcompositeglobalid.'|'.$_695localcode.'|US|3|CP_OLD_TKR|'.$field2.' US|CP_NEW_TKR|'.$field3.' US|CP_NOTES|N.A.| ' . "\r\n";
        }
//print "hi";
print "<br><br><br><br><br><br><br><br><br><br><br>";
print $csv;
$file_date =  date("Ymd");
$filename = "/var/www/html/EDI/BB_FEED/EDI-BB-LCC_translate.cax.".$file_date;
