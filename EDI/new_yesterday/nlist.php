...
$custom = "2016/08/18%";
$today = date("Y/m/d")."%";
$sql = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'NLIST' AND `changed` LIKE '$yesterday' AND `exchgcntry` LIKE 'US';";
...
for ($counter=1; $counter <= mysqli_num_rows($result); $counter++)
    ...
    default:
        ...
        $csv .= $_695localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$created.'|LIST|'.$actflag.'|'.$_695issuername.'|CUSIP|'.$uscode.'|USD|Equity| |'.$created.'|'.$date1.'|'.$chnged.'|'.$bbgcompositeglobalid.'| |'.$_695localcode.'|US|4|CP_EXCH|'.$new_exchcd.'|CP_TKR|'.$_695localcode.'|CP_SEC_TYPE|1|CP_NOTES|N.A.|' . "\r\n";
        }
//print "hi";
print "<br><br><br><br><br><br><br><br><br><br><br>";
print $csv;
$file_date =  date("Ymd");
$filename = "/var/www/html/EDI/BB_FEED/EDI-BB-NLIST_translate.cax.".$file_date;
