...
$custom = "2016/08/18%";
$today = date("Y/m/d")."%";
$sql = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'ISCHG' AND `changed` LIKE '$yesterday' AND `sectycd` = 'EQS' AND `exchgcntry` LIKE 'US';";
...
for ($counter=1; $counter <= mysqli_num_rows($result); $counter++)
        ...
        $csv .= $_695localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$created.'|CHG_NAME|'.$actflag.'|'.$_695issuername.'|N.A|N.A.|N.A.| | |'.$created.'|'.$date1.'|'.$changed.'|N.A.|'.$bbgcompositeglobalid.'|N.A.|N.A.|3|CP_OLD_NAME|'.$field2.'|CP_NEW_NAME|'.$field3.'|CP_NOTES|N.A.| ' . "\r\n";
        }
//print "hi";
print "<br><br><br><br><br><br><br><br><br><br><br>";
print $csv;
$file_date =  date("Ymd");
$filename = "/var/www/html/EDI/BB_FEED/EDI-BB-ISCHG_translate.cax.".$file_date;
