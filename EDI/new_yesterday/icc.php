...
$custom = "2016/08/18%";
$today = date("Y/m/d")."%";
$sql = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'ICC' AND `changed` LIKE '$yesterday' AND `exchgcntry` LIKE 'US';";
...
for ($counter=1; $counter <= mysqli_num_rows($result); $counter++)
        ...
        $csv .= $_695localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$created.'|CHG_ID|'.$actflag.'|'.$_695issuername.'|CUSIP|'.$uscode.'|USD|Equity| |'.$created.'|'.$date1.'|'.$changed.'|N.A.|'.$bbgcompositeglobalid.'|'.$_695localcode.'|US|5|CP_OLD_CUSIP|'.$field4.'|CP_NEW_CUSIP|'.$field5.'|CP_OLD_ISIN|'.$field2.'|CP_NEW_ISIN|'.$field3.'|CP_NOTES|N.A.|  ' . "\r\n";
        }
//print "hi";
print "<br><br><br><br><br><br><br><br><br><br><br>";
print $csv;
$file_date =  date("Ymd");
$filename = "/var/www/html/EDI/BB_FEED/EDI-BB-ICC_translate.cax.".$file_date;
