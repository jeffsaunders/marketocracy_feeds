...
$custom = "2016/08/18%";
$today = date("Y/m/d")."%";
$sql = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'BKRP' AND `changed` LIKE '$yesterday' AND `exchgcntry` LIKE 'US';";
...
for ($counter=1; $counter <= mysqli_num_rows($result); $counter++)
        ...
        $csv .= $_695localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$changed.'|BANCR|'.$actflag.'|'.$_695issuername.'|N.A.|N.A|N.A.| | |'.$created.'|'.$date1.'|'.$changed.'|N.A.|'.$bbgcompositeglobalid.'|N.A.|N.A.|2|CP_BANCR_TYP|9|CP_NOTES|N.A.| ' . "\r\n";
        }
//print "hi";
