...
$custom = "2016/08/23%";
$today = date("Y/m/d")."%";
$sql = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'SD' AND `changed` LIKE '$yesterday' AND `exchgcntry` LIKE 'US';";
...
for ($counter=1; $counter <= mysqli_num_rows($result); $counter++)
        ...
        $csv .= $_695localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$created.'|STOCK_SPLT|'.$actflag.'|'.$_695issuername.'|CUSIP|'.$uscode.'|USD|Equity| |'.$created.'|'.$date1.'|'.$changed.'|N.A.|'.$bbgcompositeglobalid.'|'.$_695localcode.'|US|9|CP_TERMS|'.$rationew.' for '.$ratioold.'|CP_RATIO|'.$CP_RATIO.'|CP_RECORD_DT|N.A.|CP_PAY_DT|N.A.|CP_STOCK_SPLT_TYP|3001|CP_ADJ|'.$CP_ADJ.'|CP_ADJ_DT|'.$date1.'| CP_SH_FRACTIONAL|N.A.|CP_NOTES|N.A.|   ' . "\r\n";
        }
//print "hi";
print "<br><br><br><br><br><br><br><br><br><br><br>";
print $csv;
$file_date =  date("Ymd");
$filename = "/var/www/html/EDI/BB_FEED/EDI-BB-SD_translate.cax.".$file_date;
