...
$custom = "2016/08/18%";
$today = date("Y/m/d")."%";
$sql = "SELECT * FROM `695_txt` WHERE `eventcd` LIKE 'PRCHG' AND `changed` LIKE '$today' AND `exchgcntry` LIKE 'US';";
...
for ($counter=1; $counter <= mysqli_num_rows($result); $counter++)
    ...
    default:
        ...
        $csv .= $_695localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$created.'|CHG_LIST|'.$actflag.'|'.$_695issuername.'|CUSIP|'.$uscode.'|USD|Equity| |'.$created.'|'.$date1.'|'.$changed.'|N.A.|'.$bbgcompositeglobalid.'|'.$_695localcode.'|US|3|CP_OLD_EXCH|'.$new_field2.'|CP_NEW_EXCH|'.$new_field3.'|CP_NOTES|N.A.|' . "\r\n";
        }
//print "hi";
