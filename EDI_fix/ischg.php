...
for ($counter=1; $counter <= mysqli_num_rows($result); $counter++)
        ...
        $csv .= $_695localcode.' US Equity|0|0|0|EDI'.$eventid.'_'.$eventcd.$date1.'|CHG_NAME|'.$actflag.'|'.$_695issuername.'|N.A|N.A.|N.A.| | |'.$created.'|'.$date1.'|'.$changed.'|N.A.|'.$bbgcompositeglobalid.'|N.A.|N.A.|3|CP_OLD_NAME|'.$field2.'|CP_NEW_NAME|'.$field3.'|CP_NOTES|N.A.| ' . "\r\n";
        }
//print "hi";
